        ...
        async with busy_lock:
            ...
            try:
                ...
                if await try_switch_on_auth_failure():
                    logger.warning("账号 Cookie 失效，已切换账号，重试 %d/%d", attempt + 1, MAX_RETRIES)
                    continue
                logger.warning("账号 Cookie 失效，无可用账号可切换")
                ...
                for stream_attempt in range(MAX_RETRIES):
                    try:
                        ...
                        if exc.status == 204 and stream_attempt == 0:
                            logger.warning("Stream 收到 204，刷新 capture 后重试一次")
                            continue
                        raise
                    except AccountAuthExpired as exc:
                        runtime_state.record(model, "errors")
                        ...
                        if not has_yielded_data and stream_attempt < MAX_RETRIES - 1 and await try_switch_on_auth_failure():
                            logger.warning("Stream 账号 Cookie 失效，已切换账号，重试 %d/%d", stream_attempt + 1, MAX_RETRIES)
                            continue
                        raise
                    except AuthError as exc:
                        if stream_attempt == 0:
                            logger.warning("Stream 鉴权异常，刷新 capture 后重试一次: %s", exc)
                            continue
                        raise
